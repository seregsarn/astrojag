...
ThpLogger *internalLogger = nullptr;

void internalLog(const char *fmt, ...) {
	va_list s;
	static char buf[4096];
	if (internalLogger == nullptr) return;

	va_start(s, fmt);
	vsnprintf(buf, 4096, fmt, s);
	va_end(s);
	internalLogger->print("thplib: %s", buf);
}
...
ThpLogger::ThpLogger(char *filename) : ThpLogger(filename, true) {}
ThpLogger::ThpLogger(char *filename, bool overwrite) {
